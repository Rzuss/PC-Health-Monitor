...
public sealed class StorageViewModel : BaseViewModel
{
    ...
    {
        _storage          = storage;
        LoadDrivesCommand = new AsyncRelayCommand(LoadDrivesAsync);
        ScanCommand       = new AsyncRelayCommand(ScanAsync, () => !IsScanning && SelectedDrive is not null);
    }

    /// <summary>Fast — just enumerate drives (called on page load).</summary>
    public ICommand LoadDrivesCommand { get; }

    /// <summary>Slow — scan large items on selected drive (only on user click).</summary>
    ...
    {
        get => _isScanning;
        set { SetProperty(ref _isScanning, value); AsyncRelayCommand.RaiseCanExecuteChanged(); }
    }
    ...
    private async Task LoadDrivesAsync()
    {
        ...
        StatusText = $"{Drives.Count} drive(s) found · Select a drive and click Find Large Items";
    }

    // ── Scan large items (user-triggered, shallow + timeout) ───────────────
...
public sealed class FolderEntry
{
    ...
    public string TypeDisplay => IsFolder ? "Folder" : "File";
